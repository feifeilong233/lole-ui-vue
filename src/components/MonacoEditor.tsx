import { defineComponent, ref, onMounted, watch, onBeforeUnmount, shallowRef, onBeforeMount } from "vue"
...
export default defineComponent({
  props: {
    ...
    onChange: {
      type: Function as PropType<(value: string, event: Monaco.editor.IModelContentChangedEvent) => any>,
      required: true,
    },
    title: {
      type: String as PropType<string>,
  ...
  setup(props) {
    console.log('%c 🥛 props: ', 'font-size:20px;background-color:#2EAFB0;color:#fff;', props);
    ...
    onMounted(() => {
      const editor = editorRef.value = Monaco.editor.create(containerRef.value, { // eslint-disabled-line
        value: props.code,
        language: 'json',
        formatOnPaste: true,
        tabSize: 2,
